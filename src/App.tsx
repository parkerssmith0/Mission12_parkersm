...
function Welcome() {
  return (<h1 style={{textAlign:"center"}}>All the schools and their mascots!!</h1>);
}

class Teams extends React.Component < {school: string, name: string, city:string, state:string } > {
  render() {
    
    const oneSchool = this.props; //Go get the properties of this specific object in memory
    
    return (
      <div className='card'>
        <h2>{oneSchool.school}</h2>
        <p>Mascot: {oneSchool.name}
        <br></br>Location: {oneSchool.city}, {oneSchool.state}</p>
...
function SchoolList() {
  return (
    <div>
        {teams.map(oneSchool => (
        <Teams 
          school={oneSchool.school}
          name={oneSchool.name} 
          city={oneSchool.city}
          state={oneSchool.state} 
        />
      ))}    </div>
  );
}
